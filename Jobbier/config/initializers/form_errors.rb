  ...
  elements.each do |e|
    if e.node_name.eql? 'label'
      if instance.error_message.kind_of?(Array)
        html = %(<div class="has-error">#{e}<span class="info-section-small">&nbsp;(#{instance.error_message.uniq.join(', ')})</span></div>).html_safe
      else
        html = %(<div class="has-error">#{e}<span class="info-section-small">&nbsp;(#{instance.error_message})</span></div>).html_safe
      end
